...
<div class="pd-20">
  <form action="" method="post" class="form form-horizontal" id="form-role-add">
  	...
  	<input type="hidden" name="id" value="${adminUser.id }"/>  <!-- 在服务器端需要编辑的id  ————  long id = Long.parseLong(req.getParameter("id"));  所以需要设置一个id -->
    <div class="row cl">
    ...
    <div class="row cl">
      ...
      <div class="formControls col-5">
        <input type="password" class="input-text" value="" placeholder="" id="password2" name="password2" recheck="password" >
      </div>
      <div class="col-4"> </div>
    </div>
    <div class="row cl">
    ...
    <div class="row cl">
      <c:forEach items="${roles }" var="role">
      	...
      	<c:if test="${zf:contains(userRoleIds,role.id) }">
      		<input type="checkbox" name="roleId" value="${role.id }" id="permId${role.id}" checked="checked" />
      	</c:if>
      	<c:if test="${not zf:contains(userRoleIds,role.id) }">
      		<input type="checkbox" name="roleId" value="${role.id }" id="permId${role.id}" />
      	</c:if>
      	
      	<label for="roleId${role.id}"><c:out value="${role.name }"/></label></div>
      </c:forEach>
    </div>
