...
public class HouseServlet extends BaseServlet {
	...
	//通过redis进行查询房源信息，(效率高)通过redis将信息保存在缓存中，下次读取会读取缓存而不是通过数据库进行读取，从而提高效率
...
//		}
		...
		HouseDTO house = (HouseDTO)cacheMgr.getValue(cacheKey, HouseDTO.class); //首先到缓存中去找
		if(house == null)//如果没有找到，则去数据库擦查询
		{
	...
	public void search(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		if (!StringUtils.isEmpty(strMonthRent)) {
			...
			query.append(" AND monthRent:[").append(strMonthRent.replace("-", " TO ")).append("]").append(" ");
		...
		if (!StringUtils.isEmpty(strRegionId)) {
			...
			query.append(" AND regionId:").append(regionId).append(" ");
		}
		Long typeId = null;
		...
		if (!StringUtils.isEmpty(keywords)) {
			...
			query.append(" AND (communityLocation:").append(keywords).append(" OR communityName:").append(keywords).append(" OR communityTraffic:").append(keywords).append(" OR ").append("decorateStatusName").append(keywords).append(" OR description:").append(keywords).append(" OR regionName:").append(keywords).append(")");
		}
	...
	public void makeAppointment(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		jedis.incr("zufangshuliang");//用计数器实现有多少人租房
		jedis.close();*/
