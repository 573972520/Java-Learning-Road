...
public class UserServlet extends BaseServlet {
	...
	public void reqSendSmsCode(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		{
			writeJson(resp, new AjaxResult("error","手机号已被注册！"));
			return;
		}
	...
	public void findPasswordSubmit1(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		String phoneNum = req.getParameter("phoneNum");
		...
		String templateId = settingService.getValue("RuPengSMS.FindPwdTemplateId");//找回密码的短信模板id
		...
		req.getSession().removeAttribute("findPassword2OK");//清除之前的中间验证标记：避免先用自己的手机号进行密码重置，造成Session中保存了findPassword2OK,然后再通过漏洞重置别人的密码
	...
	public void findPasswordSubmit3(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		//这里为什么要先将OK写前面作为判断条件呢？ —— 是因为如果将findPassword2OK写前面作为判断条件的话，那么当findPassword2OK的值为null的时候那么判断条件就没有意义了（findPassword2OK可能为null）
		{
